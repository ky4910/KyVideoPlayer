...
void VideoOutputRenderer::initGL()
{
    ...
    mShaderProgram->addShaderFromSourceFile(
        QOpenGLShader::Vertex, ":/resources/yuv.vert");

    mShaderProgram->addShaderFromSourceFile(
        QOpenGLShader::Fragment, ":/resources/yuv.frag");
...
void VideoOutputRenderer::render()
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, w, h, 0, GL_RED, GL_UNSIGNED_BYTE, currentFrame->data[0]);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, w/2, h/2, 0, GL_RED, GL_UNSIGNED_BYTE, currentFrame->data[1]);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, w/2, h/2, 0, GL_RED, GL_UNSIGNED_BYTE, currentFrame->data[2]);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
